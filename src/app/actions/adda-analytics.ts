...
import type { DailyMetric, HourDayCell } from '@/lib/adda/mock/analyticsData'
...
export async function getAddaAnalytics(addaId: string): Promise<AddaAnalyticsData | null> {
  try {
    ...
    // Fetch all past events at this adda + proposal funnel in parallel
    ...
    const hasData = dailyMetrics.length > 0 || proposalFunnel.received > 0

    return { dailyMetrics, proposalFunnel, demandGrid, hasData }
  } catch {
    return null
  }
}
