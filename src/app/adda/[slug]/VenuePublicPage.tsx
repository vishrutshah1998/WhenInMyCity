                ...
                <div className="mb-16">
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {upcomingEvents.map((event) => (
                      <Link
                        key={event.id}
                        href={`/events/${event.slug}?src=platform_discovery`}
                        className="p-5 flex items-center gap-4 no-underline transition-colors"
                        style={{ background: C.leftBg, border: `1px solid ${C.border}` }}
                      >
                        {event.cover_image_url ? (
                          <div style={{ width: 64, height: 64, position: 'relative', flexShrink: 0, overflow: 'hidden' }}>
                            <Image src={event.cover_image_url} alt={event.title} fill style={{ objectFit: 'cover' }} unoptimized />
                          </div>
                        ) : (
                          <div style={{ width: 64, height: 64, flexShrink: 0, background: C.cardBg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            <span className="material-symbols-outlined" style={{ fontSize: 24, color: C.primary }}>event</span>
                          </div>
                        )}
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p className="font-sans text-[15px] font-semibold truncate" style={{ color: C.text }}>{event.title}</p>
                          <p className="font-mono text-[11px] uppercase mt-1" style={{ color: C.textMuted }}>
                            {new Date(event.starts_at).toLocaleString('en-IN', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit', hour12: true })}
                          </p>
                          {event.maker?.display_name && (
                            <p className="font-mono text-[10px] uppercase mt-1" style={{ color: C.textFaint }}>by {event.maker.display_name}</p>
                          )}
                        </div>
              ...
              <div className="flex flex-col gap-3">
                {upcomingEvents.map((event) => (
                  <Link
                    key={event.id}
                    href={`/events/${event.slug}?src=platform_discovery`}
                    className="p-4 flex items-center gap-3 no-underline"
                    style={{ background: C.leftBg, border: `1px solid ${C.border}` }}
                  >
                    {event.cover_image_url ? (
                      ...
                      <div style={{ width: 52, height: 52, flexShrink: 0, background: C.cardBg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <span className="material-symbols-outlined" style={{ fontSize: 20, color: C.primary }}>event</span>
                      </div>
                    )}
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p className="font-sans text-[14px] font-semibold truncate" style={{ color: C.text }}>{event.title}</p>
                      <p className="font-mono text-[10px] uppercase mt-1" style={{ color: C.textMuted }}>
                        {new Date(event.starts_at).toLocaleString('en-IN', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit', hour12: true })}
                      </p>
                    </div>
                    <span className="material-symbols-outlined" style={{ fontSize: 18, color: C.primary }}>arrow_forward</span>
