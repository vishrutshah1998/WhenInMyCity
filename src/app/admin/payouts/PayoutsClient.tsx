...
import type { AdminPayoutRow } from '@/app/actions/admin'
...
export default function PayoutsClient({ payouts, currentStatus }: Props) {
  ...
  async function doAction(id: string, status: 'approved' | 'paid' | 'rejected') {
    ...
    const result = await updatePayoutStatus({ id, status, notes: actionNote[id] })
    if (!result.success) {
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 24, borderBottom: '1px solid var(--wimc-border-subtle)', paddingBottom: 0 }}>
        {STATUS_TABS.map((tab) => {
          ...
          return (
            <button
              key={tab.value}
              onClick={() => router.push(`/admin/payouts?status=${tab.value}`)}
              style={{
...
function PayoutCard({
  ...
  const makerRs = (payout.maker_paise / 100).toFixed(0)
  const grossRs = (payout.gross_paise / 100).toFixed(0)
  const platformRs = (payout.platform_paise / 100).toFixed(0)
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, marginBottom: 14 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
            <span style={{ fontWeight: 700, fontSize: 15 }}>{payout.creator_name}</span>
          ...
          <div style={{ fontSize: 12, color: 'var(--wimc-text-secondary)' }}>
            Requested {date} · {payout.event_ids.length} event{payout.event_ids.length !== 1 ? 's' : ''}
          </div>
        </div>

        {/* Amount summary */}
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--wimc-coral)', fontFamily: 'var(--font-jetbrains-mono)' }}>
            ₹{makerRs}
          </div>
          <div style={{ fontSize: 11, color: 'var(--wimc-text-muted)' }}>
            of ₹{grossRs} gross · platform ₹{platformRs}
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        {payout.bank_account && (
          <>
            <div>
              <span style={{ color: 'var(--wimc-text-muted)', marginRight: 4 }}>Bank</span>
              <span style={{ fontWeight: 600 }}>{payout.bank_name}</span>
            </div>
            <div>
      ...
      {/* Admin note */}
      {payout.notes && payout.status !== 'pending' && (
        <div style={{
