...
export default function VenueStudioClient({ adda, initialBlocks, upcomingEvents, theme: initialTheme }: Props) {
  ...
  // ── liveVenue: DB record with in-progress edits applied ────────────────────
  ...
  // ── renderVenuePanel: per-tab drawer content for the mobile StudioShell ─────
  // BlockEditor and ThemeEditor render their own save controls; only the
  // content tab needs a save bar added here.
  function renderVenuePanel(key: string): React.ReactNode {
    if (key === 'content') {
      return (
