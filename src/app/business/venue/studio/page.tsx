...
export default async function VenueStudioPage() {
  const { user } = await requireAuth('/business/venue/studio')
  const admin = createAdminClient()

  const [venueRes, blocksRes, eventsRes, profileRes] = await Promise.all([
    admin.from('venue_profiles').select('*').eq('auth_user_id', user.id).maybeSingle(),
  ...
  if (!venueRes.data) redirect('/business/venue/onboard')

  let initialBlocks = blocksRes.data ?? []
  if (initialBlocks.length === 0) {
    await seedPersonaDefaultBlocks('venue', user.id)
  ...
  const theme = resolveTheme(profileRes.data?.page_theme, { venueTypes: venueRes.data.venue_type ?? [] })
