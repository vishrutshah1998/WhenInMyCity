...
const PANEL  = 'var(--wimc-bg-elevated)'
const MUTED  = 'var(--wimc-text-secondary)'
...
// Normalized shape both getCreatorEventsWithBookings (events the creator
// hosts) and getMyTickets (events the creator attends) are flattened into at
// the page level, so this client component can render both tabs with one
// list UI instead of two.
...
interface Props {
  ...
  bio:         string | null
  hosting:     HubActivityItem[]
  attending:   HubActivityItem[]
...
const sectionLabelStyle: React.CSSProperties = {
  ...
  color: 'var(--wimc-accent)', letterSpacing: '0.2em', textTransform: 'uppercase',
  marginBottom: 14,
}

export default function CreatorProfileHubClient({
  displayName, avatarUrl, initials, username, bio, hosting, attending, workspaces, accentColor,
}: Props) {
  const [activityTab, setActivityTab] = useState<'hosting' | 'attending'>('hosting')
  const [confirmingLogout, setConfirmingLogout] = useState(false)
  const [loggingOut, setLoggingOut] = useState(false)

  const activeList = activityTab === 'hosting' ? hosting : attending
    ...
    <div style={{ maxWidth: 680, margin: '0 auto', padding: '32px 24px 80px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: bio ? 12 : 28 }}>
        <div style={{
          ...
          background: avatarUrl ? 'transparent' : `linear-gradient(135deg, ${accentColor}, ${accentColor}80)`,
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <div style={{ display: 'flex', border: `1px solid ${BORDER}`, marginBottom: 12 }}>
          {(['hosting', 'attending'] as const).map(tab => {
            ...
            const count = tab === 'hosting' ? hosting.length : attending.length
            return (
              <button
                key={tab}
              ...
              >
                {tab === 'hosting' ? 'Hosting' : 'Attending'} · {count}
              </button>
            )
          })}
        </div>
        ...
        {activeList.length === 0 ? (
          <div style={{ background: PANEL, border: `1px solid ${BORDER}`, padding: '24px 20px', textAlign: 'center' }}>
            <p style={{ fontSize: 12, color: MUTED, margin: 0 }}>
              {activityTab === 'hosting' ? 'No events hosted yet.' : 'No events booked yet.'}
            </p>
          </div>
        ) : (
              ...
              >
                <div style={{ minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: MUTED, marginTop: 2 }}>
                    {fmtDate(item.startsAt)} · {item.venueName}
                  </div>
                </div>
          ...
          <div style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button
                ...
                disabled={loggingOut}
                style={{
                  background: 'rgba(232,52,42,0.1)', border: '1px solid rgba(232,52,42,0.4)',
                  color: '#E8342A', cursor: 'pointer', padding: '7px 16px',
                  fontFamily: 'var(--font-jetbrains-mono)', fontSize: 10,
                  textTransform: 'uppercase', letterSpacing: '0.1em',
