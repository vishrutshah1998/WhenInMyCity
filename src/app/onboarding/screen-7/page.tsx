...
export default function Screen7Page() {
  ...
  useEffect(() => {
    try {
      const data = JSON.parse(sessionStorage.getItem('wimc_s1') || 'null') as Screen1Data | null
      if (!data?.creatorType) { router.replace('/onboarding/screen-1'); return }
      ...
      setS1(data)
      const saved = JSON.parse(sessionStorage.getItem('wimc_platforms') || '[]') as string[]
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 20px 88px' }}>
        <div style={{ maxWidth: 480, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          >
            <p style={{ fontFamily: 'var(--font-dm-sans)', fontSize: 17, fontWeight: 700, color: '#F0EFF8', margin: 0 }}>
              Where do you post?
            </p>
            <p style={{ fontFamily: 'var(--font-dm-sans)', fontSize: 13, color: '#9896B0', margin: '4px 0 0' }}>
              Pick up to {MAX} — we&apos;ll add them to your page.
            </p>
          </motion.div>
              ...
              return (
                <motion.button
                  ...
                  style={{
                    ...
                    gap: 6,
                    padding: '0 8px',
                    background: isSel ? `${platform.color}18` : 'rgba(255,255,255,0.03)',
                    border: `1px solid ${isSel ? platform.color : 'rgba(255,255,255,0.08)'}`,
                    borderRadius: 0,
                  ...
                  {isSel && (
                    ...
                    >
                      <svg width="7" height="6" viewBox="0 0 7 6" fill="none">
                        <path d="M1 3L2.8 4.8L6 1" stroke="white" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                    </motion.div>
                  )}
                  <div style={{ color: isSel ? platform.color : 'rgba(255,255,255,0.65)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {PLATFORM_SVG[platform.id] ?? <span style={{ fontSize: 22, lineHeight: 1 }}>{platform.emoji}</span>}
                  </div>
                  <span
                    style={{
                      ...
                      fontSize: 10,
                      fontWeight: 700,
                      color: isSel ? platform.color : 'rgba(255,255,255,0.55)',
                      textAlign: 'center',
                      lineHeight: 1.2,
        ...
        <div style={{ maxWidth: 480, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 8 }}>
          <motion.button
            ...
            style={{
              width: '100%',
              padding: '16px 24px',
              background: selected.length > 0 ? TEAL : `${TEAL}44`,
              color: '#07070A',
